...
Usage:
  ...
  python3 explain_path_decisions.py all [time_s]

  Default: from_id=1192, to_id=1204 (Tokyo→Buenos-Aires), time_s=0

  Experiment 1 pairs (from run_list.experiment1_pairs_multilayer):
  - Tokyo → Buenos-Aires:  1192 1204
  - Mumbai → Lima:         1196 1221
  - Lima → Karachi:        1221 1203

Examples:
  python3 explain_path_decisions.py 1192 1204 5.0
  python3 explain_path_decisions.py 1196 1221 0
  python3 explain_path_decisions.py 1221 1203 2.0
  python3 explain_path_decisions.py all 0
"""
...
def load_full_fstate_up_to(dynamic_state_dir, time_ns):
    ...
    the previous timestep. Merge all fstate_*.txt with time <= time_ns to get full state.
    """
...
def main():
    ...
    if args and args[0].lower() == 'all':
        run_all_pairs = True
        time_s = float(args[1]) if len(args) >= 2 else 0.0
        pairs_to_run = run_list.experiment1_pairs_multilayer  # (from_id, to_id, description)
    else:
        if len(args) >= 3:
            try:
                from_id = int(args[0])
            ...
            try:
                ...
                pass
        pairs_to_run = [(from_id, to_id, "%d→%d" % (from_id, to_id))]
    ...
    fstate_dict = load_full_fstate_up_to(dynamic_dir, time_ns)
    if not fstate_dict:
        print("ERROR: no fstate files found in", dynamic_dir)
        return 1

    for from_id, to_id, pair_desc in pairs_to_run:
        path = trace_path(fstate_dict, from_id, to_id)
        if not path or len(path) < 2:
            print("No path from %d to %d at t=%.1fs (%s)" % (from_id, to_id, time_s, pair_desc))
            if run_all_pairs:
                print()
                continue
            return 0
        ...
        print("=" * 72)
        print("Path decision explanation: %d → %d  at time %.1f s  (%s)" % (from_id, to_id, time_s, pair_desc))
